    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if($items->isEmpty())
            <div class="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <a href="{{ route('shop') }}" wire:navigate class="mt-6 inline-block bg-indigo-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-indigo-700 transition-colors">Browse Products</a>
            </div>
        @else
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-4">
                    @foreach($items as $item)
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex gap-4" wire:key="ci-{{ $item->id }}">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-3 flex items-center justify-between">
                                <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                                    <button wire:click="updateQuantity({{ $item->id }}, {{ $item->quantity - 1 }})" class="px-3 py-1.5 hover:bg-gray-50 transition-colors text-gray-600">−</button>
                        ...
                        <button wire:click="removeItem({{ $item->id }})" class="flex-none text-gray-300 hover:text-red-400 transition-colors p-1">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <button wire:click="clearCart" wire:confirm="Clear your entire cart?" class="text-sm text-red-500 hover:text-red-700 transition-colors">Clear Cart</button>
                    </div>
                </div>
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-6">
                        ...
                        <a href="{{ route('checkout') }}" wire:navigate class="mt-6 block w-full text-center bg-indigo-600 text-white py-3 px-6 rounded-xl font-semibold hover:bg-indigo-700 transition-colors">
                            Proceed to Checkout
                        </a>
                    </div>
                </div>
