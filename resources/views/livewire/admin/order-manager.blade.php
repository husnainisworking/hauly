        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors" wire:key="ord-{{ $order->id }}">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click.self="$set('showDetailModal', false)">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div class="p-6 border-b flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-bold">{{ $selectedOrder->order_number }}</h2>
                    <p class="text-sm text-gray-500">{{ $selectedOrder->created_at->format('M d, Y H:i') }}</p>
                </div>
                <button wire:click="$set('showDetailModal', false)" class="text-gray-400 hover:text-gray-600 text-xl">✕</button>
            </div>
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="text-sm">{{ $selectedOrder->shipping_address['address'] ?? '' }}, {{ $selectedOrder->shipping_address['city'] ?? '' }}, {{ $selectedOrder->shipping_address['state'] ?? '' }}</p>
                    </div>
                    <div>
