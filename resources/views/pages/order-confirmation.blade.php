...
new class extends Component {
    ...
    {
        $this->order = Order::where('order_number', $orderNumber)->with('items.product')->firstOrFail();
    }
};
?>
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="text-center mb-10">
        ...
        <p class="mt-2 text-gray-600">Thank you for your purchase. We've received your order.</p>
                    ...
                    <div class="flex items-center gap-4">
                        @if($item->product)
                            <img src="{{ $item->product->primary_image }}" alt="{{ $item->product_name }}"
                                class="w-14 h-14 object-cover rounded-xl flex-none" />
                        @endif
                        <div class="flex-1">
    ...
    <div class="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
        <a href="{{ route('home') }}" wire:navigate class="px-8 py-3 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700 transition-colors text-center">
            Continue Shopping
        </a>
        @auth
            <a href="{{ route('profile.orders') }}" wire:navigate class="px-8 py-3 border border-gray-300 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-colors text-center">
                View My Orders
            </a>
        @endauth
    </div>
